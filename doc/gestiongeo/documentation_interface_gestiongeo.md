...
* l'équipe des géomètres: pour créer des nouveaux dossiers suite à la réception de levé pour pouvoir mettre à jour les bases TA_LIG_TOPO_F et TA_POINT_TOPO_F.
* l'équipe des photos-interprète: pour créer des dossiers de vérification à l'intention des géomètres.
...
Cet onglet permet de créer des dossiers qui sont à des stades de recolements différents. Si les données prochainement intégrées dépassent le périmètre, celle-ci est redessinée.
...
* plan de recolement: plan qui décrit les travaux réellement effectués à la fin d'un chantier.
* IC: Investigation Complémentaire, investigation pour connaitre avec précision la localisation et la nature des réseaux soutterains situés sur la parcelle ou des travaux s'effectuent.
* MAJ carto: demande de modification par les consultants (les photos-interprètes par exemple)
...
* copier les fichiers *dwg* renseigné dans le champ *choisir le fichier dwg à intégrer* sur infogeo (***infogeo/appli_gg/recol/***)
* **A noter que FME peut remplacer le point de l'extension fichier par un tiret-bas. Dans ce cas là, il est possible de corriger cette erreur en modifiant l'extension des fichier directement dans le repertoire infogeo/appli_gg/recol/XXX_dwg**
...
* **TA_LIG_TOPO_GPS**, 
* **TA_POINT_TOPO_GPS**
après vérification sont bonnes, leurs attributs ***GEO_ON_VALIDE*** est mis à 0 et sont copiées dans les tables:
* **TA_LIG_TOPO_F**
* **TA_POINT_TOPO_F**
...
Cette icône permet de connaitre les dossiers créés par les UT et à valider le cas échéant. Il permet aussi de connaitre les dossiers rejetés. Les UT peuvent créer des dossiers afin de mettre à jour des éléments qu'ils relèvent. Cette fonction n'est plus plus utilisée.
...
Avant d'intégrer les plans par l'intermédiaire de **GESTIONGEO** les plans sont controlés sur *autocad*. Est controlé sur *autocad*
