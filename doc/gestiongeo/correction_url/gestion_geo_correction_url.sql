...
COMMENT ON TABLE "G_DALC"."TEMP_GG_FILES_LIST"  IS 'Table de test regroupant les fichiers et leurs liens d''acces  pour chaque dossier de l''application GESTIONGEO.';
COMMENT ON COLUMN "G_DALC"."TEMP_GG_FILES_LIST"."LIEN" IS 'Lien vers les fichiers exportes depuis INFOGEO.';
COMMENT ON COLUMN "G_DALC"."TEMP_GG_FILES_LIST"."OBJECTID" IS 'Clé primaire de la table TEST_GG_FILES_LIST.';
COMMENT ON COLUMN "G_DALC"."TEMP_GG_FILES_LIST"."ID_DOS" IS 'Numéro ID_DOS du dossier de l''application GESTIONGEO auxquel se rattache le fichier';
COMMENT ON COLUMN "G_DALC"."TEMP_GG_FILES_LIST"."INTEGRATION" IS 'Indication si le fichier a été intégré ou pas. Si 1 le fichier a été intégre. O non.';
...
UPDATE G_DALC.TEMP_GG_FILES_LIST_3
...
-- 5.3.1. MISE A 1 DE LA COLONNE POUR LES DOSSIERS OU UN SEUL FICHIER EST PRESENT
