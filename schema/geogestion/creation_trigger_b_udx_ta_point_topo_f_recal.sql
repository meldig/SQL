...
create or replace TRIGGER B_UDX_TA_POINT_TOPO_F_RECAL
    BEFORE UPDATE OR DELETE ON GEO.TA_POINT_TOPO_F_RECAL
    FOR EACH ROW
DECLARE
    username varchar(30);
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                mail.sendmail('geotrigger@lillemetropole.fr',username || ' a provoque l''erreur : ' || SQLERRM,'ERREUR TRIGGER - geo/prod.B_UDX_TA_POINT_TOPO_F_RECAL','bjacq@lillemetropole.fr');
    END;