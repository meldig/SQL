...
-- Procédure pour detecter les lignes avec des problèmes de géométrie



CREATE OR REPLACE PROCEDURE MISE_A_JOUR_Z_TA_RTGE_LINEAIRE_INTEGRATION_DETECTION_ERREUR(IDENTIFIANT_DOSSIER NUMBER)
AS
BEGIN
    -- Suppression des données de la table contenant les lineaire en erreur
    DELETE FROM G_GESTIONGEO.TA_RTGE_LINEAIRE_ERREUR;  
    ...
    USING 
        ...
        SELECT
            ...
            t.Y as COORD_Y,
            t.Z as COORD_Z,
            SDO_GEOMETRY(3001, 2154,SDO_POINT_TYPE(t.X,t.Y,t.Z),NULL, NULL) as GEOM
        FROM
    ...
    INSERT INTO G_GESTIONGEO.TA_RTGE_LINEAIRE_ERREUR(OBJECTID, FID_IDENTIFIANT_TYPE, FID_NUMERO_DOSSIER,GEOM)
    ...
    FROM
        G_GESTIONGEO.TA_RTGE_LINEAIRE_INTEGRATION a
        INNER JOIN G_GESTIONGEO.TA_GG_RECUPERATION_Z_ETAPE_11 b ON b.OBJECTID = a.OBJECTID
    WHERE
        b.geom.sdo_gtype <> 3002;

END MISE_A_JOUR_Z_TA_RTGE_LINEAIRE_INTEGRATION_DETECTION_ERREUR;
