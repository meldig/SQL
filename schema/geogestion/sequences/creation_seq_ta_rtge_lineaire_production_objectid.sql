-- 4. Sequence SEQ_TA_RTGE_LINEAIRE_PRODUCTION_OBJECTID de la table TA_RTGE_LINEAIRE_PRODUCTION
...
-- recupere le dernier numero de la colonne de clé primaire +1 pour determiner le nouveau point de départ de la séquence.
    BEGIN
      ...
      ;
    DBMS_OUTPUT.PUT_LINE('CREATE SEQUENCE SEQ_TA_RTGE_LINEAIRE_PRODUCTION_OBJECTID START WITH ' || NOMBRE_DEPART || ' INCREMENT BY 1 NOCACHE');
    END;
-- execution de la requete
    BEGIN
      EXECUTE IMMEDIATE
      'CREATE SEQUENCE SEQ_TA_RTGE_LINEAIRE_PRODUCTION_OBJECTID START WITH ' || NOMBRE_DEPART || ' INCREMENT BY 1 NOCACHE';
    END;
END;
