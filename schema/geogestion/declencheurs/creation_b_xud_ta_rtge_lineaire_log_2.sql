-- 2. Creation du trigger B_XUD_TA_RTGE_LINEAIRE_LOG
...
CREATE OR REPLACE TRIGGER G_GESTIONGEO.B_XUD_TA_RTGE_LINEAIRE_LOG
BEFORE UPDATE OR DELETE ON G_GESTIONGEO.TA_RTGE_LINEAIRE
FOR EACH ROW
DECLARE
USERNAME VARCHAR(30);
...
-- TRIGGER
    IF UPDATING THEN -- En cas de modification on insère les valeurs de la table TA_RTGE_LINEAIRE_LOG, le numéro d'agent correspondant à l'utilisateur, la date de modification et le type de modification.
        INSERT INTO G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG(GEOM, IDENTIFIANT_OBJET, FID_NUMERO_DOSSIER, FID_IDENTIFIANT_TYPE, DECALAGE_DROITE, DECALAGE_GAUCHE, FID_PNOM_ACTION, DATE_ACTION, FID_TYPE_ACTION)
            VALUES(
            		:old.GEOM,
					:old.objectid,
    ...
    EXCEPTION
        WHEN OTHERS THEN
            mail.sendmail('rjault@lillemetropole.fr',SQLERRM,'ERREUR TRIGGER - G_DALC.B_XUD_TA_RTGE_LINEAIRE_LOG','rjault@lillemetropole.fr');
END;
