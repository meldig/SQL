/*
creation trigger de migration: B_IUX_TA_RTGE_LINEAIRE_MIGRATION
*/
...
CREATE OR REPLACE TRIGGER B_IUX_TA_RTGE_LINEAIRE_MIGRATION
...
OBJET := :new.OBJECTID;
	...
	IF :new.ETAT_INTEGRATION = 1 THEN
		...
		INSERT INTO G_GESTIONGEO.TA_RTGE_LINEAIRE(GEOM, FID_NUMERO_DOSSIER, FID_IDENTIFIANT_TYPE, DECALAGE_DROITE, DECALAGE_GAUCHE, FID_IDENTIFIANT_OBJET_INTEGRATION)
		VALUES (
				:new.GEOM,
	...
	EXCEPTION
	WHEN OTHERS THEN
	    mail.sendmail('rjault@lillemetropole.fr',SQLERRM || ' Sur l''objet : ' || OBJET || ' modifié par ' || USERNAME,'ERREUR TRIGGER - G_GESTIONGEO.B_XUX_TA_RTGE_LINEAIRE_MIGRATION','rjault@lillemetropole.fr');
END;
