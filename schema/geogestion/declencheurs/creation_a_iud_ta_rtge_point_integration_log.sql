-- 2. Creation du trigger A_XUD_TA_RTGE_LINEAIRE_INTEGRATION_LOG
...
CREATE OR REPLACE TRIGGER G_GESTIONGEO.A_XUD_TA_RTGE_LINEAIRE_INTEGRATION_LOG
AFTER UPDATE OR DELETE ON G_GESTIONGEO.TA_RTGE_LINEAIRE_INTEGRATION
FOR EACH ROW
DECLARE
USERNAME VARCHAR(30);
...
-- TRIGGER
    IF INSERTING THEN -- En cas de modification on insère les valeurs de la table TA_RTGE_LINEAIRE_INTEGRATION_LOG, le numéro d'agent correspondant à l'utilisateur, la date de modification et le type de modification.
    ...
    EXCEPTION
        WHEN OTHERS THEN
            mail.sendmail('rjault@lillemetropole.fr',SQLERRM,'ERREUR TRIGGER - G_DALC.A_XUD_TA_RTGE_LINEAIRE_INTEGRATION_LOG','rjault@lillemetropole.fr');
END;
