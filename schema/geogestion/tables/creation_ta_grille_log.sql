...
COMMENT ON COLUMN G_GESTIONGEO.TA_GRILLE_LOG.id_gestionnaire IS 'Identifiant de la table TA_GG_SOURCE permettant d''attribuer le pnom d''un agent à un élément de la grille afin d''indiquer qu''il doit s''en occuper.';
...
COMMENT ON COLUMN G_GESTIONGEO.TA_GRILLE_LOG.fid_pnom IS 'Clé étrangère vers la table TA_GG_SOURCE permettant d''associer à chaque objet le pnom de l''agent l''ayant créé ou modifié.';
