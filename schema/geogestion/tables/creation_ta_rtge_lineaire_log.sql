...
COMMENT ON COLUMN G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG.FID_IDENTIFIANT IS 'IIdentifiant de l''objet qui est/était présent dans la table de production.';
COMMENT ON COLUMN G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG.NUMERO_DOSSIER IS 'Reference du dossier pour lequel l''objet a ete releve';
COMMENT ON COLUMN G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG.IDENTIFIANT_TYPE IS 'Identifiant de la classe de l''objet';
...
COMMENT ON COLUMN G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG.FID_PNOM_MODIFICATION IS 'Dernier agent qui a modifie l''objet';
COMMENT ON COLUMN G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG.DATE_MODIFICATION IS 'Date de derniere modification de l''objet';
COMMENT ON COLUMN G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG.MODIFICATION IS 'Type de modification effectuée sur la donnée : 1 = mise à jour, 0 = suppression';
...
ADD CONSTRAINT TA_RTGE_LINEAIRE_LOG_OBJECTID_PK 
...
ALTER TABLE G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG
ADD CONSTRAINT TA_RTGE_LINEAIRE_LOG_IDENTIFIANT_TYPE_FK 
...
ALTER TABLE G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG
ADD CONSTRAINT TA_RTGE_LINEAIRE_LOG_PNOM_MODIFICATION_FK 
...
ALTER TABLE G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG
ADD CONSTRAINT TA_RTGE_LINEAIRE_LOG_NUMERO_DOSSIER_FK 
...
-- 3.2.6.2. Création d'un index sur le champ IDENTIFIANT_TYPE
CREATE INDEX TA_RTGE_LINEAIRE_LOG_IDENTIFIANT_TYPE_IDX ON G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG(IDENTIFIANT_TYPE) TABLESPACE G_ADT_INDX;

-- 3.2.6.3. Creation d'un index sur le champ numero dossier
CREATE INDEX TA_RTGE_LINEAIRE_LOG_NUMERO_DOSSIER_IDX ON G_GESTIONGEO.TA_RTGE_LINEAIRE_LOG(NUMERO_DOSSIER) TABLESPACE G_ADT_INDX;
