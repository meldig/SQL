/*
Création de la table ADMIN_COMMUNES_MEL90 qui regroupe toutes les communes de la MEL90 issues de la BdTopo2019.
*/
-- 1. Création de la table
CREATE TABLE G_REFERENTIEL.ADMIN_COMMUNES_MEL90(IDENTIFIANT, CODE_INSEE, NOM, GEOM)
AS
SELECT
...
WHERE
    ...
    AND g.debut_validite = '01/01/17'
    AND g.fin_validite = '31/12/19'
    AND UPPER(m.nom_source) = UPPER('bdtopo')
    AND UPPER(o.acronyme) = UPPER('ign')
;

-- 2. Création des commentaires de VM et des colonnes
COMMENT ON TABLE G_REFERENTIEL.ADMIN_COMMUNES_MEL90 IS 'Table proposant les communes de la MEL90 extraites de la BdTopo2019 de l''IGN.';
COMMENT ON COLUMN G_REFERENTIEL.ADMIN_COMMUNES_MEL90.IDENTIFIANT IS 'Clé primaire de la table.';
...
COMMENT ON COLUMN G_REFERENTIEL.ADMIN_COMMUNES_MEL90.GEOM IS 'Géométrie de chaque commune.';

-- 3. Création de la clé primaire
ALTER TABLE G_REFERENTIEL.ADMIN_COMMUNES_MEL90 
ADD CONSTRAINT ADMIN_COMMUNES_MEL90_PK 
...
CREATE INDEX ADMIN_COMMUNES_MEL90_SIDX
ON G_REFERENTIEL.ADMIN_COMMUNES_MEL90(GEOM)
...
CREATE INDEX ADMIN_COMMUNES_MEL90_CODE_INSEE_IDX ON G_REFERENTIEL.ADMIN_COMMUNES_MEL90(CODE_INSEE)
    TABLESPACE G_ADT_INDX; 
CREATE INDEX ADMIN_COMMUNES_MEL90_NOM_IDX ON G_REFERENTIEL.ADMIN_COMMUNES_MEL90(NOM)
    TABLESPACE G_ADT_INDX;
...
GRANT SELECT ON G_REFERENTIEL.ADMIN_COMMUNES_MEL90 TO G_REFERENTIEL_MAJ;
