...
FROM
    ...
    /*INNER JOIN G_GEO.TA_METADONNEE k ON k.objectid = c.fid_metadonnee
    INNER JOIN G_GEO.TA_SOURCE l ON l.objectid = k.fid_source
    INNER JOIN G_GEO.TA_METADONNEE_RELATION_ORGANISME m ON m.fid_metadonnee = k.objectid
    INNER JOIN G_GEO.TA_ORGANISME n ON n.objectid = m.fid_organisme
    INNER JOIN G_GEO.TA_DATE_ACQUISITION o ON o.objectid = k.fid_acquisition*/
WHERE
    UPPER(f.valeur) = UPPER('code insee')
    AND UPPER(i.acronyme) = UPPER('mel')
    AND sysdate BETWEEN g.debut_validite AND g.fin_validite
    /*AND UPPER(l.nom_source) = UPPER('bdtopo')
    AND UPPER(n.acronyme) = UPPER('ign')*/;
