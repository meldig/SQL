...
COMMENT ON COLUMN g_geo.ta_commune.insee IS 'Code INSEE de chaque commune.';
COMMENT ON COLUMN g_geo.ta_commune.nom_commune IS 'Nom complet de chaque commune.';
COMMENT ON COLUMN g_geo.ta_commune.geom IS 'Géométrie de chaque commune.';
...
COMMENT ON COLUMN g_geo.ta_commune.fid_lib_type_commune IS 'Clé étrangère permettant de connaître le statut de la commune - ta_libelle.';
...
ALTER TABLE ta_commune
ADD CONSTRAINT ta_commune_fid_lib_FK
FOREIGN KEY (fid_lib_type_commune)
REFERENCES ta_libelle(objectid);

-- 7. Création des index sur les clés étrangères
CREATE INDEX ta_commune_fid_source_IDX ON ta_commune(fid_metadonnee)
    TABLESPACE G_ADT_INDX;

CREATE INDEX ta_commune_fid_lib_IDX ON ta_commune(fid_lib_type_commune)
    TABLESPACE G_ADT_INDX;
...
COMMENT ON COLUMN g_geo.ta_zone_administrative.acronyme IS 'Nom de chaque zone administrative supra-communale.';
