...
-- 5. Création des index sur les cleffs étrangères.
...
COMMENT ON COLUMN ta_bpe_relation_libelle.fid_libelle_parent IS 'Composante de la clef primaire. Clef étrangère vers la table TA__LIBELLE pour connaitre le libelle parent.';
...
CREATE INDEX ta_bpe_caracteristique_fid_libelle_IDX ON ta_bpe_caracteristique(fid_libelle_fils)
    TABLESPACE G_ADT_INDX;

CREATE INDEX ta_bpe_caracteristique_fid_libelle_IDX ON ta_bpe_caracteristique(fid_libelle_parent)
    TABLESPACE G_ADT_INDX;
...
-- 4. Création de la séquence d'auto-incrémentation de la clef primaire
