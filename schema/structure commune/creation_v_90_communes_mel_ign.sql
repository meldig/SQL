...
CREATE OR REPLACE FORCE VIEW g_referentiel.adm_communes_90_mel (
    objectid,
    nom_minuscule,
    nom_majuscule,
    code_insee,
    ...
    aire_km2,
    source,
    CONSTRAINT "adm_communes_90_mel_PK" PRIMARY KEY ("CODE_INSEE") DISABLE
)
AS
 WITH
 v_main_selection AS(
...
COMMENT ON TABLE g_referentiel.adm_communes_90_mel IS 'Vue proposant les communes actuelles de la MEL extraites de la BdTopo de l''IGN.';
COMMENT ON COLUMN g_referentiel.adm_communes_90_mel.NOM_MINUSCULE IS 'Nom de chaque commune de la MEL en minuscule.';
COMMENT ON COLUMN g_referentiel.adm_communes_90_mel.NOM_MAJUSCULE IS 'Nom de chaque commune de la MEL en majuscule.';
COMMENT ON COLUMN g_referentiel.adm_communes_90_mel.CODE_INSEE IS 'Code INSEE de chaque commune.';
COMMENT ON COLUMN g_referentiel.adm_communes_90_mel.CODE_POSTAL IS 'Code Postal de chaque commune.';
COMMENT ON COLUMN g_referentiel.adm_communes_90_mel.GEOM IS 'Géométrie de chaque commune - de type polygone.';
COMMENT ON COLUMN g_referentiel.adm_communes_90_mel.SOURCE IS 'Source de la donnée avec l''organisme créateur de la source.';