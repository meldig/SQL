...
CREATE OR REPLACE TRIGGER ta_diff_carto_lidar
...
BEGIN
    ...
    IF :new.fid_lib_etat = '3' AND :new.fid_thematique = '1' THEN
       -- Tous les objets bâtis se trouvant à l'intérieur de l'élément de la grille voient leur statut passer à '1', c-a-d "traité".
    ...
    WHEN OTHERS THEN
        mail.sendmail(
            'bjacq@lillemetropole.fr',
            SQLERRM,'ERREUR TRIGGER ta_diff_carto_lidar',
            'trigger@lillemetropole.fr'
        );
END;