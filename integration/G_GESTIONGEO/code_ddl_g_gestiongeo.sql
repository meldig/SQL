...
-- 1.4. Les indexes
CREATE INDEX G_GESTIONGEO."TA_GG_SOURCE_SRC_LIBEL_IDX" ON G_GESTIONGEO.TA_GG_SOURCE ("SRC_LIBEL") 
ON TABLESPACE G_ADT_INDX;
...
CREATE TABLE G_GESTIONGEO.TA_GG_ETAT (
	...
	"ETAT_SMAL" VARCHAR2(25 BYTE) NOT NULL
);
...
-- 2.4. Les indexes
CREATE INDEX G_GESTIONGEO."TA_GG_ETAT_ETAT_AFF_IDX" ON G_GESTIONGEO.TA_GG_ETAT ("ETAT_AFF") 
ON TABLESPACE G_ADT_INDX;

CREATE INDEX G_GESTIONGEO."TA_GG_ETAT_ETAT_LIB_SMALL_IDX" ON G_GESTIONGEO.TA_GG_ETAT ("ETAT_LIB_SMALL") 
ON TABLESPACE G_ADT_INDX;

CREATE INDEX G_GESTIONGEO."TA_GG_ETAT_ETAT_SMALL_IDX" ON G_GESTIONGEO.TA_GG_ETAT ("ETAT_SMALL") 
ON TABLESPACE G_ADT_INDX;

-- 2.5. Affectation des droits
...
CREATE INDEX G_GESTIONGEO."TA_GG_FAMILLE_FAM_LIB_SMALL_IDX" ON G_GESTIONGEO.TA_GG_FAMILLE ("FAM_LIB_SMALL") 
ON TABLESPACE G_ADT_INDX;
...
CREATE TABLE G_GESTIONGEO.TA_GG_GEO (
	"ID_GEOM" NUMBER NOT NULL,
	"ID_DOS" NUMBER NOT NULL,
	"CLASSE_DICT" VARCHAR2(1 BYTE) NULL,
...
COMMENT ON COLUMN G_GESTIONGEO.TA_GG_GEO.CLASSE_DICT IS 'Identifiant des réseaux contenus dans la charte topographique et réseaux de la DSIG';
...
FOREIGN KEY("USER_ID")
REFERENCES G_GESTIONGEO.TA_GG_SOURCE ("USER_ID");
...
ALTER TABLE G_GESTIONGEO.TA_GG_GEO
ADD	CONSTRAINT TA_GG_GEO_ID_DOS_DOS_NUM_FK
FOREIGN KEY ("ID_DOS", "DOS_NUM") REFERENCES G_GESTIONGEO.TA_GG_DOSSIER ("ID_DOS", "DOS_NUM") ON DELETE CASCADE;

-- 5.4. Les indexes
CREATE INDEX G_GESTIONGEO."TA_GG_DOSSIER_ETAT_ID_FK" ON G_GESTIONGEO.TA_GG_DOSSIER ("ETAT_ID") 
ON TABLESPACE G_ADT_INDX;

CREATE INDEX G_GESTIONGEO."TA_GG_DOSSIER_FAM_ID_FK" ON G_GESTIONGEO.TA_GG_DOSSIER ("FAM_ID") 
ON TABLESPACE G_ADT_INDX;

CREATE INDEX G_GESTIONGEO."TA_GG_DOSSIER_USER_ID_FK" ON G_GESTIONGEO.TA_GG_DOSSIER ("USER_ID") 
ON TABLESPACE G_ADT_INDX;

CREATE INDEX G_GESTIONGEO."TA_GG_DOSSIER_SRC_ID_FK" ON G_GESTIONGEO.TA_GG_DOSSIER ("SRC_ID") 
ON TABLESPACE G_ADT_INDX;
...
COMMENT ON TABLE G_GESTIONGEO.VM_GG_POINT IS 'Vue matérialisée rassemblant les centroïdes de chaque périmètre de dossier de GestionGeo. Les sélections se font sur les tables TA_GG_GEO et TA_GG_DOSSIER.';
