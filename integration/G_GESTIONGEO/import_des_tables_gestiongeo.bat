...
:: 5.5. table TA_GG_GEO
ogr2ogr.exe -f OCI OCI:%USER_D%/%MDP_D%@%INSTANCE_D% OCI:%USER_P%/%MDP_P%@%INSTANCE_P% -sql "SELECT * FROM GEO.TA_GG_GEO" -nln TEMP_TA_GG_GEO -nlt MULTIPOLYGON -lco SRID=2154 -dim 2

:: 5.6. table TA_GEO_CORRECT_DOUBLONS
ogr2ogr.exe -f OCI OCI:%USER_D%/%MDP_D%@%INSTANCE_D% OCI:%USER_P%/%MDP_P%@%INSTANCE_P% -sql "SELECT * FROM GEO.TA_GEO_CORRECT_DOUBLONS" -nln TEMP_TA_GEO_CORRECT_DOUBLONS
