...
ogr2ogr.exe -f OCI OCI:%USER_D%/%MDP_D%@%INSTANCE_D% OCI:%USER_P%/%MDP_P%@%INSTANCE_P% -sql "SELECT * FROM GEO.TA_GG_DOSSIER_SAVE" -nln TEMP_TA_GG_DOSSIER

:: 5.5. table TA_GG_GEO
ogr2ogr.exe -f OCI OCI:%USER_D%/%MDP_D%@%INSTANCE_D% OCI:%USER_P%/%MDP_P%@%INSTANCE_P% -sql "SELECT * FROM GEO.TA_GG_GEO_SAVE" -nln TEMP_TA_GG_GEO -nlt MULTIPOLYGON -lco SRID=2154 -dim 2
