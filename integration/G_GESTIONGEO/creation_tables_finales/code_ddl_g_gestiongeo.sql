...
-- 10. Trigger permettant l'insertion des OBJECTID, USER_ID et DOS_DMAJ
...
BEGIN
    ...
    IF INSERTING THEN -- En cas d'insertion on insère le OBJECTID correspondant à l'utilisateur dans TA_GG_DOSSIER.OBJECTID        
        :new.OBJECTID := v_OBJECTID;
        :new.DOS_DC := TO_DATE(sysdate, 'dd/mm/yy');
    END IF;

    IF UPDATING THEN -- En cas d'édition on insère le OBJECTID correspondant à l'utilisateur dans TA_GG_DOSSIER.USER_ID et on édite le champ DOS_DMAJ
