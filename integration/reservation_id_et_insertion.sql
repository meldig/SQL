...
-- Rajout d'un champ affectant un id à chaque valeur à insérer dans la table d'import + incrémentation de ce champ
...
-- Insertion des valeurs dans la table d'insertion
MERGE INTO G_GEO.TEMP_LIBELLE_LONG a
    USING(
            SELECT 
                ...
                b.valeur
            FROM
                G_GEO.TEMP_LIBELLE_POINT_VIGILANCE b
        )t
    ON(a.valeur = t.valeur)
WHEN NOT MATCHED THEN
    INSERT(a.objectid, a.valeur)
    VALUES(t.fid_objectid, t.valeur);
COMMIT;
...
-- Rajout d'un champ affectant un id à chaque valeur à insérer dans la table d'import + incrémentation de ce champ
EXECUTE IMMEDIATE 'ALTER TABLE G_GEO.TEMP_PNOM ADD fid_objectid NUMBER(38,0) GENERATED BY DEFAULT AS IDENTITY (START WITH ' || v_id_max || ' CACHE 20)';
