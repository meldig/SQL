...
USING
    ...
    WITH annee AS (
        ...
        WHERE
            b.date_acquisition = TO_DATE(SYSDATE)
        AND b.millesime BETWEEN '01/01/1876' AND '01/01/2017'
        AND b.nom_obtenteur = SYS_CONTEXT('USERENV','OS_USER')
        AND c.nom_source = 'Recensements de la population 1876-2017'
        AND c.description = 'Les statistiques sont proposées dans la géographie communale en vigueur au 01/01/2019 pour la France hors Mayotte, afin que leurs comparaisons dans le temps se fassent sur un champ géographique stable.'
