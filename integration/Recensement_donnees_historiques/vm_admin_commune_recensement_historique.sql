...
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.identifiant IS 'Clé primaire de la vue.';
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.code_insee IS 'Code INSEE de chaque commune.';
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.nom IS 'Nom de chaque commune de la MEL.';
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.recensement IS 'Type de recensement';
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.description_recensement IS 'Description du recensement';
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.population IS 'Nombre d''habitant au sein de la commune calculé sur la base de la géographie des communes en 2019.';
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.source IS 'Information sur les metadonnées des données utilisées pour créer la vue matérialisée.';
COMMENT ON COLUMN G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE.geom IS 'Géométrie de chaque commune - de type polygone.';
...
VALUES(
    'VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE',
    'geom',
...
ALTER MATERIALIZED VIEW G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE 
ADD CONSTRAINT ADMIN_COMMUNE_RECENSEMENT_PK
PRIMARY KEY (IDENTIFIANT,RECENSEMENT)
USING INDEX TABLESPACE "G_ADT_INDX";


-- 5. Création de l'index spatial
CREATE INDEX VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE_SIDX
ON G_REFERENTIEL.VM_ADMIN_COMMUNE_RECENSEMENT_HISTORIQUE(GEOM)
