...
WITH 
	...
	FROM
		cte 
	INNER JOIN G_GEO.ta_libelle_correspondance tc_niv_0 ON cte.fid_libelle_niv_0 = tc_niv_0.fid_libelle 
	INNER JOIN G_GEO.ta_libelle_correspondance tc_niv_1 ON cte.fid_libelle_niv_1 = tc_niv_1.fid_libelle
	INNER JOIN G_GEO.ta_libelle_correspondance tc_niv_2 ON cte.fid_libelle_niv_2 = tc_niv_2.fid_libelle
	INNER JOIN G_GEO.ta_libelle_correspondance tc_niv_3 ON cte.fid_libelle_niv_3 = tc_niv_3.fid_libelle
	...
	INNER JOIN G_GEO.ta_libelle tl_niv_0 ON cte.fid_libelle_niv_0 = tl_niv_0.objectid
	INNER JOIN G_GEO.ta_libelle tl_niv_1 ON cte.fid_libelle_niv_1 = tl_niv_1.objectid
	INNER JOIN G_GEO.ta_libelle tl_niv_2 ON cte.fid_libelle_niv_2 = tl_niv_2.objectid
	INNER JOIN G_GEO.ta_libelle tl_niv_3 ON cte.fid_libelle_niv_3 = tl_niv_3.objectid
	...
	INNER JOIN
		G_GEO.ta_famille tf_niv_3 ON tfl_niv_3.fid_famille = tf_niv_3.objectid
	WHERE
	    tlc_niv_0.valeur = 'CS'
    	OR 
		tlc_niv_0.valeur = 'US'
...
indice AS
	...
	FROM
	    ...
	    INNER JOIN G_GEO.TA_LIBELLE_CORRESPONDANCE e ON e.fid_libelle = d.OBJECTID
	    INNER JOIN G_GEO.TA_LIBELLE_COURT f ON f.OBJECTID = e.fid_libelle_court
	    INNER JOIN G_GEO.TA_LIBELLE_LONG g ON g.objectid = d.fid_libelle_long
	    INNER join G_GEO.TA_FAMILLE_LIBELLE h on h.fid_libelle_long = g.objectid
	    inner join G_GEO.TA_FAMILLE i ON i.objectid = h.fid_famille
	WHERE
...
ocs2d_attribut AS (
	...
	FROM
		...
		LEFT JOIN G_OCS2D.TA_OCS2D_RELATION_COMMENTAIRE ocr ON ocr.fid_ocs2d_millesime = b.objectid
		INNER JOIN millesime mi ON mi.objectid = a.objectid
		LEFT JOIN G_OCS2D.TA_OCS2D_COMMENTAIRE oc ON oc.objectid = ocr.fid_ocs2d_commentaire
		INNER JOIN G_GEO.TA_METADONNEE m ON b.fid_metadonnee = m.objectid
        INNER JOIN G_GEO.TA_SOURCE s ON s.objectid = m.fid_source
        INNER JOIN G_GEO.TA_DATE_ACQUISITION da ON da.objectid = m.fid_acquISition
        INNER JOIN G_GEO.TA_PROVENANCE p ON p.objectid = m.fid_provenance
